...
{
/* add code here */
/* Iterate through the list of mappings for this process and remove the given
  mapping.*/
list_remove(&m->elem);

 /*We need to iterate through every page in the memory mapped file. */
for(int i = 0; i < m->page_cnt; i++)
{
  /*  If this page has been modified (is dirty), write it to the disk so changes
  are saved.*/
 ...
 /* Now that we have determined dirty pages and written them to the disk,
 we can iterate through all pages in the memory mapped file and deallocate
 them, which frees up process memory!*/
for(int i = 0; i < m->page_cnt; i++)
{
...
{
/* add code here */
/* Use lookup_mapping to get the map corresponding to the given map id.
  Call unmap, which we implemented, to try to unmap this mapping.*/
